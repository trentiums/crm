    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.leads.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('company_user_id') ? 'is-invalid' : '' }}" type="number" name="company_user_id" id="company_user_id" value="{{ old('company_user_id', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead_channel') ? 'is-invalid' : '' }}" name="lead_channel_id" id="lead_channel_id" required>
                    @foreach($lead_channels as $id => $entry)
                        <option value="{{ $id }}" {{ old('lead_channel_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_services') ? 'is-invalid' : '' }}" name="product_services[]" id="product_services" multiple required>
                    @foreach($product_services as $id => $product_service)
                        <option value="{{ $id }}" {{ in_array($id, old('product_services', [])) ? 'selected' : '' }}>{{ $product_service }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead_conversion') ? 'is-invalid' : '' }}" name="lead_conversion_id" id="lead_conversion_id" required>
                    @foreach($lead_conversions as $id => $entry)
                        <option value="{{ $id }}" {{ old('lead_conversion_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time_line') ? 'is-invalid' : '' }}" type="text" name="time_line" id="time_line" value="{{ old('time_line', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('deal_amount') ? 'is-invalid' : '' }}" type="number" name="deal_amount" id="deal_amount" value="{{ old('deal_amount', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('win_close_reason') ? 'is-invalid' : '' }}" type="text" name="win_close_reason" id="win_close_reason" value="{{ old('win_close_reason', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('deal_close_date') ? 'is-invalid' : '' }}" type="text" name="deal_close_date" id="deal_close_date" value="{{ old('deal_close_date') }}">
